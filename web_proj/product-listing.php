...
<?php
    ...
    $server_query = trim(str_replace('cache=1','',$server_query));
    $first_char = substr($server_query, 0, 1);
    if ($first_char == "&" || $first_char == "?")
        $server_query = substr($server_query, 1, strlen($server_query));
    //var_dump($server_query);
    ...
    $exec_time = round($elapsed_time, 3);

    $data = $json["data"];
    $entity_resolution_time = $is_cached ? 0 : round($json['attributes']['entityResolutionTime'] / 1000, 3);
    $server_exec_time = $is_cached ? 0 : round($json['attributes']['totalTime'] / 1000, 3);
    $download_time = $is_cached ? 0 : round($json['attributes']['downloadTime'] / 1000,3);
        ...
        try {
            ...
            if ($counter == 0) {
                DB::insert(CACHE_TABLE, array(
                    ...
                    'entity_resolution_time'=> $entity_resolution_time
                ));
            }
            else {
...
<?php } ?>
    ...
    <div class="table-responsive" style="width:100%;clear:both">
        <table class="table table-hover table-bordered">
            <tr>
                <th class="table-header">Total Time</th>
                <th class="table-header">Server Execution</th>
                <th class="table-header">Download Time</th>
                <th class="table-header">Resolution Time</th>
            ...
            <tr>
                <td class="table-data"><?php echo $exec_time;?> sec</td>
                <td class="table-data"><?php echo $server_exec_time;?> sec</td>
                <td class="table-data"><?php echo $download_time;?> sec</td>
                <td class="table-data"><?php echo $entity_resolution_time;?> sec</td>
